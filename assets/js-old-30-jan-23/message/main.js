$(document).ready(function () {
	...
	function getUserList() {
		return new Promise(function (resolve, reject) { //Creating new Promise Chain
			...
			document.getElementById('user_list2').innerHTML = data; //setting data to the user list
		  
	 
			$.get(base_url+'Message/ownerDetails', function (data) {
	...
	// Self user
	 function selfUser(){
		...
		return new Promise(function (resolve, reject) { //Creating new Promise Chain
			...
			$(document).on('click', '.selfuser', function (event) {
				...
				bg_image  =  $(this).data('user_avtar');
				unique_id=$(this).data('uniqueid');
				universe= $(this).data('universe')!='' ?  $(this).data('universe'):'0';
	...
	function getUserDetails(uniq_id) {
		$.post('Message/getIndividual', { data: uniq_id ,universe:universe}, function (data) {
	...
	function sendUserUniqIDForMsg(uniq_id, bg_image) {
		$.post('getmessage', { data: uniq_id, user_avtar: bg_image,universe:universe }, function (data) {
	...
	function DeleteSelfMessage(){
	 
		$.ajax({
			url:  base_url +'Message/deleteSelfMessage',
			type: 'get',
			success: function (res) {
				console.log(res);
	...
	setInterval(DeleteSelfMessage, 10000); 
	//send message after button click
  // Create the audio object
  var audio = new Audio('https://talkmos.com/assets/sound/send.mp3');
 
	$('#send_message').click(function (e) {
		$('#count_text').html('0/200');
		audio.play();
		var d = new Date(),
			...
			// $('#messageText').css('background','green')
...
$('#talkToUniverse').on('click',function(){

$(document).ready(function () {
	...
	function getUserList() {
		return new Promise(function (resolve, reject) { //Creating new Promise Chain
			...
			$('.innerBox').click(function () {
				...
				getBlockUserData();
				setInterval(getBlockUserData, 500000);

				getUserDetails(unique_id);
				inter2 = setInterval(getUserList, 500000);
	...
	$('#search').keyup(function (e) {
		...
		if (val.length > 0) {
			...
			inter2 = setInterval(getUserList, 5000000);
		}
	});
	...
	inter2 = setInterval(getUserList, 5000000); //Calling the root function with interval
})
